...
export default class PlayScene extends Phaser.Scene {
    ...
    create() {
        ...
        // 初期以降のマップ
        this.mapKeys = ['flatMap',]; 
        this.initMap();     // 最初のマップロード
    ...
    update(time, delta) {
        if (!this.isPaused) {
            ...
            // すべてのプラットフォーム速度更新
            // それぞれのマップオブジェクト（preMap, curMap, postMap）のレイヤー群にアクセスして位置を変更
            if (this.curMap.layers['ground'].x < - this.curMap.width / 6) {
                this.updateMap();
            }
            var mapList = [this.preMap, this.curMap, this.postMap]
            if (this.curMap.layers['ground'].x < 0) mapList = [this.curMap, this.postMap]
            mapList.forEach(mapObj => {
                if (mapObj && mapObj.layers) {
                    Object.values(mapObj.layers).forEach(layer => {
                        // レイヤーが存在する場合、その位置を変更
                        if (layer) {
                            layer.x -= this.platformSpeed * deltaTime;
                        }
                    });
                }
            });
            // this.player.setVelocityX(this.platformSpeed);
            // this.cameras.main.scrollX += this.platformSpeed * deltaTime;
        ...
        // 地面にいる場合
        // if (this.player.body.blocked.down) {
        //     this.player.setVelocityX(0);  // 横方向に動かす（platformStartSpeedと同じ速度）
        // } else {
        //     this.player.setVelocityX(0);  // 空中にいる間は横方向に動かさない
        // }
        this.player.setVelocityX(0);  // platform間に摩擦がない

        // プレイヤーのx位置がgameOptions.playerStartPosition / 2 より小さくなった場合 かつ　地面に接している時
        if ((this.player.x < gameOptions.playerStartPosition / 2 || this.player.x > gameOptions.playerStartPosition* 3 / 2)  && (this.player.body.blocked.down)) {
        ...
        if (this.player.body.blocked.down && !this.isPaused) {
    ...
    initMap() {
        // 最初に既存のマップを削除
        if (this.preMap) {
            this.preMap.layers && Object.values(this.preMap.layers).forEach(layer => layer.destroy());
            this.preMap = null;
        }
        if (this.curMap) {
            this.curMap.layers && Object.values(this.curMap.layers).forEach(layer => layer.destroy());
            this.curMap = null;
        }
        if (this.postMap) {
            this.postMap.layers && Object.values(this.postMap.layers).forEach(layer => layer.destroy());
            this.postMap = null;
        }
        var initMapKeys = ['startMap', 'startMap', 'flatMap'];     //map切り替え用のキー群
        // 最初に startMap を表示
        this.preMap = this.loadMap(initMapKeys[0], 0);
        this.curMap = this.loadMap(initMapKeys[1], this.preMap.layers['ground'].x + this.preMap.width);
        this.postMap = this.loadMap(initMapKeys[2], this.curMap.layers['ground'].x + this.curMap.width);
        // this.nextMapIndex = 3; // mapKeysの次に読み込む位置
    }
    loadMap(mapKey, x) {
        const map = this.make.tilemap({ key: mapKey });
        const tileset = map.addTilesetImage('Tiles1', 'Tiles1');
        const layers = {};
        let layerNames = ['Background', 'Ground', 'Deco',]; // 使いたいレイヤー名を列挙
        layerNames.forEach(name => {
            if (map.getLayer(name)) {
                const layer = map.createLayer(name, tileset, x, 0);
                layers[name.toLowerCase()] = layer;
                // Groundレイヤーだけに衝突判定（名前で判断）
                if (name === 'Ground') {
    ...
    updateMap() {
        ...
        // 🚨 curMap の位置と幅を保存しておく（使う前に）
        const offsetX = this.curMap.layers['ground'].x + this.curMap.width;
    
        // 各マップをスライド
        this.preMap = this.curMap;
        this.curMap = this.postMap;
    
        // 次のマップキーを取得（ランダム）
        const nextKey = Phaser.Math.RND.pick(this.mapKeys);
    
        // 新しいマップを読み込み
        this.postMap = this.loadMap(nextKey, offsetX); // ← 保存しておいた offsetX を使う
    }    
